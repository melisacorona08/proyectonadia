...
class MathplayApp(MDApp):
    ...
    def _generar_certificado(self, nota, etapa):
        ...
        ancho, alto = letter
        c = rl_canvas.Canvas(nombre_archivo, pagesize=letter)

        # Borde decorativo simple
        c.setStrokeColorRGB(0.09, 0.64, 0.29)  # verde
        c.setLineWidth(4)
        c.rect(30, 30, ancho - 60, alto - 60)

        # Título
        c.setFont("Helvetica-Bold", 28)
        c.setFillColorRGB(0.08, 0.33, 0.17)
        c.drawCentredString(ancho / 2, alto - 110, "CERTIFICADO DE LOGRO")

        # Subtítulo
        c.setFont("Helvetica", 16)
        c.setFillColorRGB(0.3, 0.3, 0.3)
        c.drawCentredString(ancho / 2, alto - 150, "Mathplay Academy")

        # Línea separadora
        c.setStrokeColorRGB(0.09, 0.64, 0.29)
        c.setLineWidth(1.5)
        c.line(80, alto - 165, ancho - 80, alto - 165)

        # Cuerpo
        c.setFont("Helvetica", 14)
        c.setFillColorRGB(0.2, 0.2, 0.2)
        c.drawCentredString(ancho / 2, alto - 210, "Se certifica que:")

        c.setFont("Helvetica-Bold", 22)
        c.setFillColorRGB(0.08, 0.33, 0.17)
        c.drawCentredString(ancho / 2, alto - 255, self.usuario_actual)
        ...
        c.drawCentredString(ancho / 2, alto - 380, f"Calificación: {nota}/10")

        # Fecha
        c.setFont("Helvetica", 13)
        c.setFillColorRGB(0.4, 0.4, 0.4)
        fecha = datetime.now().strftime("%d de %B de %Y")
        c.drawCentredString(ancho / 2, alto - 420, f"Fecha: {fecha}")

        # Firma simulada
        c.setFont("Helvetica-BoldOblique", 14)
        c.setFillColorRGB(0.08, 0.33, 0.17)
        c.drawCentredString(ancho / 2, alto - 490, "Mathplay Academy")
        c.setFont("Helvetica", 11)
        c.setFillColorRGB(0.5, 0.5, 0.5)
        c.drawCentredString(ancho / 2, alto - 510, "Plataforma Educativa de Matemáticas")
